...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	for {
		...
		if slotResult == openAISlotAcquireProfitVetoed {
			...
			recordOpenAIProfitVeto(failedAccountIDs, account.ID)
			continue
		}
		...
		if isGrokVideoCreateEndpoint(endpoint) && strings.TrimSpace(result.ResponseID) != "" {
			...
			pending := service.GrokVideoPendingBilling{
				Model:                requestModel,
				BillingModel:         firstNonEmptyGrokMediaString(result.BillingModel, requestModel),
...
func prepareGrokVideoCompletionBilling(
	...
	taskRequestID = strings.TrimSpace(firstNonEmptyGrokMediaString(taskRequestID, statusResult.ResponseID))
	if taskRequestID == "" {
		return nil
	}
	...
	if pending != nil {
		if strings.TrimSpace(merged.Model) == "" {
			merged.Model = firstNonEmptyGrokMediaString(pending.BillingModel, pending.Model, pending.OriginalModel)
		}
		if strings.TrimSpace(merged.BillingModel) == "" {
			merged.BillingModel = firstNonEmptyGrokMediaString(pending.BillingModel, pending.Model, merged.Model)
		}
	...
	merged.RequestID = service.StableGrokVideoBillingRequestID(firstNonEmptyGrokMediaString(merged.ResponseID, taskRequestID))
	merged.ResponseID = firstNonEmptyGrokMediaString(merged.ResponseID, taskRequestID)
	merged.VideoCount = 1
...
func firstNonEmptyGrokMediaString(values ...string) string {
...
func recordGrokMediaUsage(
	...
	if result != nil && result.VideoCount > 0 {
		videoTaskID = strings.TrimSpace(firstNonEmptyGrokMediaString(requestID, result.ResponseID))
		if stable := service.StableGrokVideoBillingRequestID(firstNonEmptyGrokMediaString(result.ResponseID, requestID)); stable != "" {
