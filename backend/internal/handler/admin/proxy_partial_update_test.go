...
func TestProxyHandlerUpdatePreservesFieldPresence(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, tc.set, svc.input.ExpiresAt.Set)
			require.Equal(t, tc.set, svc.input.BackupProxyID.Set)
			require.Equal(t, tc.warn, svc.input.ExpiryWarnDays.Set)
			if !tc.set || tc.clear {
				require.Nil(t, svc.input.ExpiresAt.Value)
				require.Nil(t, svc.input.BackupProxyID.Value)
			} else {
				require.Equal(t, int64(1800000000), svc.input.ExpiresAt.Value.Unix())
				require.Equal(t, int64(10), *svc.input.BackupProxyID.Value)
			}
		})
	}
}
