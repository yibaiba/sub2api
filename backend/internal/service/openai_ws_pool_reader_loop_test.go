...
func (d *openAIWSReaderLoopFakeDialer) Dial(context.Context, string, http.Header, string, int64) (openAIWSClientConn, int, http.Header, error) {
...
func TestOpenAIWSConnReaderLoop_ReadTimeoutAbortsWithoutCloseHandshake(t *testing.T) {
	...
	ws, _, _, err := newDefaultOpenAIWSClientDialer().Dial(context.Background(), "ws"+strings.TrimPrefix(server.URL, "http"), nil, "", 0)
	require.NoError(t, err)
