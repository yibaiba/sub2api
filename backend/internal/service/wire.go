...
const (
	imageDownloadRequestTimeout        = 60 * time.Second
...
func ProvideImageStorageSettingService(
	...
	cfg *config.Config,
) (*ImageStorageSettingService, error) {
	...
	downloadClient, err := sharedhttp.GetClient(sharedhttp.Options{
		Timeout:               imageDownloadRequestTimeout,
		ResponseHeaderTimeout: imageDownloadResponseHeaderTimeout,
		PinResolvedIP:         true,
	})
	if err != nil {
		return nil, fmt.Errorf("create image download client: %w", err)
	}
	settings := NewImageStorageSettingService(settingRepo, encryptor, backup, factory, cfg.ImageStorage)
	settings.SetDownloadClient(downloadClient)
	return settings, nil
}
