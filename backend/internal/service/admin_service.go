...
type CreateUserInput struct {
	...
	Username             string
	Notes                string
	Role                 string // 空字符串表示使用默认角色(user);合法值 admin/user/usage_viewer
	Balance              *float64
	Concurrency          int
	RPMLimit             int
	AllowedGroups        []int64
...
type UpdateUserInput struct {
	...
	Username        *string
	Notes           *string
	Role            string   // 空字符串表示"未提供"(不修改);合法值 admin/user/usage_viewer
...
type CreateProxyInput struct {
	...
	FallbackMode   string
	BackupProxyID  *int64
	ExpiryWarnDays OptionalIntInput
}
...
type OptionalInt64Input struct {
	Set   bool
	Value *int64
}
...
type UpdateProxyInput struct {
	...
	FallbackMode   OptionalStringInput
	BackupProxyID  OptionalInt64Input
	ExpiryWarnDays OptionalIntInput
}
